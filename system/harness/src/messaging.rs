...
impl MessagingHandler {
    ...
    /// Fetch all agent-type messages for `agent_id` with status=new (or in_progress for
    /// crash recovery). Transitions them to status=in_progress atomically so that if
    /// the wake crashes mid-cycle, the next wake re-delivers them.
    pub fn receive(&self, agent_id: &str) -> Result<Vec<Message>, String> {
        let mut messages = self.read_messages()?;
        let now = Utc::now().to_rfc3339();
        let mut received: Vec<Message> = Vec::new();

        for msg in &mut messages {
            if msg.msg_type != MessageType::Agent {
                continue;
            }
            if !msg.to.iter().any(|r| r == agent_id) {
                continue;
            }
            if msg.status == "new" {
                msg.status = "in_progress".to_string();
                msg.action_log.push(ActionEntry {
                    ts: now.clone(),
                    action: format!("received:wake:{}", agent_id),
                    related_assets: Vec::new(),
                });
                received.push(msg.clone());
            } else if msg.status == "in_progress" {
                // Crash recovery: re-deliver interrupted messages on next wake.
                received.push(msg.clone());
            }
        }
    ...
    /// Mark messages as delivered after a SUCCESSFUL wake cycle. Called at wake-end,
    /// NOT at wake-start — ensures agents that crash or error mid-wake retry on next wake.
    /// Only call this when wake_succeeded is true.
    pub fn mark_delivered(&self, ids: &[String], agent_id: &str) -> Result<(), String> {
        if ids.is_empty() {
            return Ok(());
        }
        let mut messages = self.read_messages()?;
        ...
        for msg in &mut messages {
            if ids.contains(&msg.id) {
                ...
                if !msg.routed_to.contains(&agent_id.to_string()) {
                    msg.routed_to.push(agent_id.to_string());
                }
                msg.action_log.push(ActionEntry {
                    ts: now.clone(),
                    action: format!("delivered:wake:{}", agent_id),
                    related_assets: Vec::new(),
                });
                changed = true;
            }
