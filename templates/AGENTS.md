...
> **BOI is PAUSED (owner decision, 2026-09-15: unreliable in practice; Standing Order 6).** Do not dispatch BOI specs. Build multi-step work in Claude Code: inline edits in a worktree (SO 7), `Agent` subagents on Sonnet (SO 3b), or a `Workflow`. Testing standard applies. Resume needs the owner's explicit call plus a decision file. The BOI material below is parked reference.
